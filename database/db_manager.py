...
class DBManager:
    ...
    async def get_or_create_daily_temp_email(self, user_id: int, api_key: str) -> str:
        ...
        try:
            ...
            session.rollback()
            logger.error(f"Error managing daily temp email for user {user_id}: {e}")
            raise
        except Exception as e:
            logger.error(f"Error in get_or_create_daily_temp_email for user {user_id}: {e}")
            raise
        finally:
            session.close()

# Initialize DBManager globally
db_manager = DBManager()  # <-- ضع هذا السطر هنا في نهاية الملف
